...
关于需求与问题的界定，如下哪些说法是正确的？ABCD
A 新产品/版本相对于老产品/版本的特性、设计等遗漏，应属于问题
B 已引发网上事故的设计和实现，属于问题
C 新产品/版本相对于老产品/版本的性能，在相同场景下出现下降，应属于问题
D 属于公司/产品线/产品等DFx基线的需求，当前版本不具备，应属于需求1、对软件需求的分类，以下理解错误的是：（）
A 质量需求也应属于软件需求，在需求分析时需要考虑
B 系统必须提供版本间兼容的北向接口属于版本管理范畴，不应该纳入软件需求进行分析
C 软件需求可以分为功能性需求，质量需求和约束
D 不明确的功能性需求，在需求分析时不需要细化
参考答案： BD1、需求分析输出的质量准则包括完整性，正确性，可验证性，一致性；其中正确性包括如下哪些准则：（）
A 正确的，相关涉众确认了需求的正确性，并且系统完整地实现了文档化的需求
B 最新的，需求反应了系统当前的实现状态
C 无歧义的，需求的文档化描述只允许一种合理的解释
D 原子的，每个需求描述的是单一，内聚的事实。
参考答案： ABCD2、在面向团队进行需求收集时，下列需求收集方法的叙述不恰当的是：（）
A 举行团队需求收集会议，会议由软件工程师、客户和其他利益相关者共同举办和参加
B 拟定一个会议议程，与会者围绕需求要点，畅所欲言
C 会议提倡自由发言，不需要特意控制会议的进度
D 会议目的是为了识别问题，提出解决方案的要点，初步刻画解决方案中的需求问题
参考答案： C1、软件需求管理是软件项目开发过程中控制和维持需求约定的活动，以下哪些属于需求管理活动：（）
A 需求发现
B 需求变更控制
C 版本控制
D 需求跟踪
参考答案： BCD
19	12  隐私保护
个人数据（PersonalData）：自然人的email地址、电话号码、生物特征（指纹）、位置数据、IP地址、医疗信息、宗教信仰、社保号、婚姻状态等。
敏感个人数据(Sensitive Personal Data)：欧盟法律：种族、政治观点、宗教和哲学信仰、工会成员资格、基因数据、生物信息、健康和性生活状况、性取向等。
g定义的敏感个人数据包括：在原有基础之上新增银行卡号、身份证号、护照号、口令等。 隐私保护越来越多引起大家的重视，以下对GDPR描述错误的是（ ）C
A. GDPR（General Data Protection Regulation, “GDPR”），即《通用数据保护法》
B. GDPR授权一个执法机构统一执法，且违法的行政处罚最高可高达2000万欧元，或企业全球营收4%，以较高者为准
C. GDPR对欧盟境外实体没有任何约束，不论该实体是否向欧盟境内数据主体提供服务、处理欧盟境内数据主体的个人数据
D. GDPR对数据控制者和数据处理者都有法定义务的要求以下关于隐私数据相关设计的描述，哪些设计是错误的( ) BD
A. 匿名化是一种对数据不可逆改变的过程，处理后的个人数据将无法直接或间接地还原，已经不再是个人数据，因此匿名化后的数据不受个人数据保护法律法规的限制。
B. 假名化是一种数据保护的手段（如偏移、置换、哈希等），假名化数据不是个人数据，因此假名化以后的数据不受个人数据保护法律法规的限制。
C. 对于需要存储的个人数据，在设计时应根据收集和使用目的确定存留期。
D. 考虑易用性，系统默认打开个人数据收集便于启动个性化运维功能，用户可以通过设置关闭。
. 一般来说，认证有三种方式：他知道的内容（something you know）、他持有的证明（something you have）、他就是这个人（something you are）。它们也常称作“根据知识进行认证、根据所有权进行认证以及根据特征进行认证”。指纹认证属于哪一种认证方式（ ）A
A. 他就是这个人（something you are）
B. 他持有的证明（something you have）
C. 其它选项都不对
D. 他知道的内容（something you know）
（多选）敏感个人数据包括哪些（ ）
A. 敏感个人数据
B. 银行卡号
C.身份证号
...
20.3	13.3   会话的生命周期
阶段	问题	解决办法
认证前	DDoS	根据帐号、角色、IP 地址、时间等特征来确定是否允许通过认证建立认证后的会话
限制或禁止同一用户同时建立多个会话
	基于IP	 
	基于应用层协议攻击	 
会话生成	会话标识可猜测攻击
(使用不安全随机数算法生成会话标识会话标识的长度不够)	使用长id 破解DdoS攻击
	会话固定攻击	1.	B/S 应用中，用户名和口令认证通过后，必须更换会话标识
2.	所有登录后才能访问的界面都必须提供主动退出选项，当用户退出时，服务器端必须清除该用户的会话信息
3.	必须设置会话超时机制，在超时过后必须要清除该会话信息	会话劫持	1.	会话标识需要进行安全传输
2.	B/S 应用中，会话cookie的属性要设置为HttpOnly
3.	B/S 应用中，cookie中正确设置“Secure”、域和路径	敏感操作无验证	1.   验证码  2.TOKEN
会话维持	会话标识暴露	1-B/S 应用中，必须使用cookie 用于维持会话
2-禁止在URL、错误信息或日志中暴露会话标识符 	会话信息泄露	1-会话使用过程中，会话信息必须在服务器端存储和维护
2-服务器需要结合sessionid、IP地址以及其他的会话信息对每一个会话进行身份校验，判断该会话是否合法
会话销毁问题	会话未销毁	1.	所有登录后才能访问的界面都必须提供主动退出选项，当用户退出时，服务器端必须清除该用户的会话信息
2.	必须设置会话超时机制，在超时过后必须要清除该会话信息 	 	 20.4	13.4   题目
  系统在会话过程中往往存在较多安全问题，关于会话管理错误的设计是（）A
A. 系统支持的用户同时建立的会话数越多越好 --限制或禁止同一用户同时建立多个会话
B. 会话标识必须使用安全随机数算法生成（且有效长度不低于24个字符）
C. B/S 应用中，用户名和口令认证通过后，必须更换会话标识
D. 会话标识需要进行安全传输
  会话前攻击  ABC
A  基于IP
B  基于上层业务
...
A. 校验http referrer 
B. 校验cookie的token
C. 校验cookie的会话ID21	14  密码学
21.1	14.1   基础
1、禁止MD5应用在参与生成“数字签名、口令加密保存”这两种场景（HMAC-MD5例外），禁止SHA1应用在参与生成“数字签名”的场景。
2、无需还原的口令必须使用PBKDF2或公司密码学专家组认可的更全的算法加密
3、系统的管理平面和近端维护终端、网管维护终端间，支持使用合适的安全协议（如SSH v2/TLS1.1/TSL1.2/IPSec/SFTP/ SNMPv3等）；对于公司指定的已知不安全的FTP、Telnet和SSHv1协议应支持关闭。
21.2	14.2   题目
  在数字信封加密方式下，真正用来加解密通信过程中所传输数据（明文）的密钥是（）B
A. CA中心的公钥
B. 对称加密密钥
C. 非对称算法的公钥
D. 非对称算法的私钥
数字信封是将对称密钥通过非对称加密（即：有公钥和私钥两个）的结果分发对称密钥的方法。
数字信封是指发送方使用（A）来加密对称密钥后所得的数据？
A.     接收方的公钥 B.发送方的公钥 C.接收方的私钥 D.发送方的私钥
  （多选）禁止采用如下哪些算法进行口令加密（BD）
A. RSA
B. DES
C. SKIPJACK
D.MD5
解析：DES和MD5是绝对禁止的，RSA和SKIPJACK在1024以下是禁止的但是不是绝对禁止（多数答案没有C）禁止使用业界已知不安全的加密算法,DES/3DES（除密钥K1≠K2≠K3外的场景）/SKIPJACK/RC2/RSA（1024位及以下）/MD2/MD4。
禁止MD5应用在参与生成“数字签名”/口令
  提供手段保证“密钥”的安全性，防止密钥被盗取或泄露，以下说法正确的有BCD
A 安全协议采用私有算法，确保私钥的安全性
B OPENSSH通过口令保护私钥
C LINUX2.6引入了密钥保留服务
D HSM对密钥提供硬件保护下面哪些是安全的随机数产生器ABDE
A  类Unix平台的/dev/random文件
B  OpenSSL库的EAND_bytes
C  java的java.uti.Random
D  Windows平台的CyptGenRandom
E  JDK的java.security.SecureRandom
F  C 库函数random()
  OpenSSL库的RAND_bytes
 中软iPSI组件的CRYPT_random(需开启DRBG)
 中软VPP的IPSI_CRYPT_rand_bytes
 JDK的java.security.SecureRandom
 类Unix平台的/dev/random文件
 Windows平台的CryptGenRandom
 中软TEE的TEE_GenerateRandom  下面哪个说法是正确的  ABCD
A 对称算法和非对称算法与安全性无关，不能说哪一种更安全
B 密钥的本质，应满足随机性的要求；口令、默认设置的可见字符等，均不能直接作为密钥使用
C禁止使用SSL2.0、SSL3.0、TLS1.0协议
D禁止用Hash(key||message)方式，即将密钥与消息进行直接拼接再进行哈希运算当做MAC来使用
 
为以下流程图所示的程序设计一组测试用例，下面（）满足判定/条件覆盖标准的最小的测试数据组是? (B)A.      (X=0,Y=0); (X=1,Y=1); (X=15,Y=9); (X=17,Y=11)
B.      (X=0,Y=0),(X=17,Y=11)【？？】
C.      (X=0,Y=0), (X=1,Y=0), (X=17,Y=6)
D.      (X=0,Y=0), (X=1,Y=0), (X=15, Y=9), (X=17,Y=6)
解释 
过度的使用断言检查每个细节，致使整个测试变得脆弱【主要是无关的内容】，可考虑通过: (BC) 
以下哪些属于公司15个关键架构能力目标（ABCD） 下列描述中（ACD）是编码可测试性好的标准
A.      提供查询系统状态的接口。比如内存使用、程序使用进程数等。
B.      一个模块直接访问另一个模块的内部数据。
C.      使用模块化方法，编码低耦合、高内聚。
D.      数据的显示与控制实现有效分离。AC
EC【Max(Pi)】某系统有三个输入A,B,C，每个输入的取值个数分别为4 6 7，使用BC（基本选择组合）方法设计测试用例，至少需要设计（C）个用例
A 17
B 3
C 15
D 7
BC测试法的测试用例个数 = SUM(Pi) – M + 1. 这里SUM(Pi) = 4+6 +7， 由于只有3个输入，所以M=3， 那么结果很显然是15.单元测试的主要任务包括（ACD） 
针对是否对无效数据进行测试（AB） 下面关于性能测试的说法（BD）是正确的题解：A. TPS不能直接反映系统性能。
      C. 应该选择最佳并发数，这里正好反了。 
关于逻辑覆盖，下列说法正确的有：(ABCD)
A 判断覆盖无法保证一定能查出判断条件存在的出错误
B 100%的判定条件覆盖可以保证100%的条件覆盖
C 100%的条件覆盖不一定满足100%的判定覆盖
D 100%的判定覆盖无法保证100%的语句覆盖如果输入条件规定了整型参数的范围及默认值，依据边界值分析方法（ACDE）可以作为测试数据
A 最大值+1
B 随机值
C 最小值
D 最大值
E 最小值-1
F 默认值
2（多选）下面代码中，哪些用例可以100%达到语句覆盖、条件覆盖、判定覆盖
void someFuntion() {
         // other code
if (x > 5 && y < 10) {
    doSomething();
...
}A.      (x = 2, y = 10, z = 15)( x = 10, y = 5, z = 20) 两个用例可以100%达到条件覆盖【NY，YY，表达式内容都走了】
B.      (x = 10, y = 5, z = 20) 一个用例可以100%达到语句覆盖
C.      (x = 1, y = 10, z = 15)( x = 10, y = 5, z = 20) 两个用例可以100%达到条件覆盖【NN，YY】
D.      (x = 2, y = 10, z = 15)( x = 10, y = 5, z = 10) 两个用例可以100%达到判定覆盖【NY，YN】1.在测试的系统(通过因特网订购书)中，有下面三个参数:
Number of Books：(Few, Many)
Sum : (Low, Middle, High)
Membership card (None, Silver Card, Gold Card)
如果用pair-wise方法设计测试用例，需要()个测试用例可以满足要求()
...
关于重构，以下说法正确的有（BC）重构不一定是最佳方案
以下代码，存在哪些坏味道: (BC)
  A. 依恋情结，对参数i作了大量的charAt操作。
  B. 烂用注释，注释没有自注释，并且注释内容与实际意义不符。
  C. 命名不规范，不能表达实际意思。
  D. 过度耦合消息链。反回值过度依赖入参细节。public String getType(String i) {
    ...
    for (int j = 0; j < i.length(); j++) {
        ...
        if (j == 0) {
            ...
            if (i.charAt(0) == '=') {
                ...
                k.append(String.valueOf(i.charAt(0)));
            }
        }
    }
    return k.toString();
}某产品有代码中有个超大函数，有850行代码18个参数和31个局部变量，哪些方法可以减少函数代码量并减少参数和局部变量 （AB）【这时候得 】
A 通过提炼函数方法，提取一批功能内聚，名称通俗易懂的函数
B 引入参数对象，把参数封装成一个或几个类，将参数封装成对象后提高了代码的可读性
C 将函数内联化将封装不良的下层函数直接展开到当前函数中
D 为了提高可读性，决定引入解释性变量，把某些复杂表达式结果放到命名良好的临时变量里面【越来越复杂】 
22.1	能识别产品整体架构的坏味道。（1题）Duplicated Code（重复的代码 ）
抽取函数到合适的类中
重构方法：提炼函数、函数上移、提炼类塑造模板函数【改名没有用】
Long Method （过长函数）
函数不应该超过50行，否则可能职责不单一，要尝试分离
...
修改类或则方法的原因不止一个，职责可能就不单一了
重构方法：提炼类
Shotgun Surgery（霰弹式修改）

一个需求变更需要修改多处，职责可能被分到多处，应该将这些需要修改的地方尝试放到一处
重构方法：搬移函数、搬移值域
Feature Envy（依恋情结）

例如，某个函数为了计算某值，从另一个对象那儿调用几乎一半的取值函数（getting method）。可以考虑将该方法分解成小方法，并将小方法迁移到合适的地方。
重构方法：搬移函数、提炼函数

Data Clumps（数据泥团）

当看到一些数据项经常会一起出现的时候，应该将他们封装成一个类的属性。好处是将参数缩短，减少传入参数，简化函数的调用动作
重构方法：提炼类、引入参数对象、保持对象完整


Primitive Obsession（基本类型偏执）

不要执着于使用基本类型，当类型间处在较多关系时，应该考虑将他们用类组织起来使用，作为类的对象出现
重构方法：以对象取代数据值、以类取代型别码、以子类取代型别码

Switch Statements（ switch 惊悚现身）

switch语句的问题在于重复（duplication）。你常会发现同样的switch语句散布于不同地点。如果要为它添加一个新的子句，
你必须找到所有switch语句并修改它们。可以考虑使用多态解决这个问题，减少switch语句的使用
重构方法：以子类取代型别码、以State/strategy 取代型别码、以多态取代条件式
Parallel Inheritance Hierarchies（平行继承体系）

属于散弹式修改的特殊情况，在这种情况下，每当你为某个类增加子类时，必须为另外的一个类也增加相应的子类。
解决方法：让一个继承体系的实例引用另一个继承体系的实例。如果再接再厉运用 Move Method （搬移函数）
和Move Field （搬移字段）,，就可以将引用端的继承体系取消。

Lazy Class （冗赘类）某些类的职责已经不值得单独提取成一个类
解决方法： 将其职责转移或声明成内部类
重构方法：折叠继承关系、将类内联化
...
对象中存在令人迷惑的属性，其存在仅仅是为了某些特殊使用的场景而设置，如仅在执行某些算法才生效。
可以将这些属性和使用的地方抽离成单独的类，method object（译注：其存在只是为了提供调用函数的途径，class 本身并无抽象意味）。
Message Chains（过度耦合的消息链）

函数调用过长，有一连串的getThis()或一长串临时变量，可以考虑使用Hide Delegate方式。增加新的方法，将消息链截短，隐藏调细节
重构方法：隐藏委托关系
...
两个函数做同一件事情却拥有不同的函数签名，应该通过重命名或迁移函数等方法解决



Incomplete Library Class（不完美的程序库类）、工具库不够完美的情况下可以考虑外加函数 （Introduce Foreign Method） 或 引入本地扩展（Introduce Local Extension）
Data Class （ 纯稚的数据类）

只拥有一些些值域（fields），以及用于访问（读写）这些 值域的函数。需要检查是否恰当封装，是否存在不该被外部改值得情况却被更改。
属性被调用的方法是否可以移到该类中
Refused Bequest（被拒绝的遗赠）

子类不想全部继承父类的东西，此时应该考虑父类是否过大，是否应该再抽象一层。
重构方法：下移方法、下移字段
Comments（过多的注释）

代码过多注释。代码注释过多的原因往往是因为代码不易阅读，过于复杂。
重构方法：提炼函数、重命名当拆解一个大函数时，对于拆解出来的小函数我们根据函数所要达到的目（即做什么）的来进行命名。
函数搬移方法：当发现类A某个函数不仅仅适用于当前类，还适用于其他类时，该函数抽取为独立函数放入工具中，供其他类使用。
被拒绝的遗赠：子类不需要父类的数据，方法，会导致滥用继承，造成理解困难和不必要的耦合。
异曲同工的类：两个函数做同样的事情却有两个不同的签名，缺乏抽象，会引发不必要的复杂度。
数据泥团：数据总是扎堆出现，扎堆消失。解决办法：把数据抽取成对象。
过长的参数列表：分割成小函数，用对象来封装。
过大的类：抽取子类，提取新类。
基本类型偏执：改用结构类型来抽取概念的时候没用，非要用基本数据类型，不愿意为了小对象建立对象，比如数组中的下标代表了不同的含义。解决办法：使用对象替代。
冗余类：不必要存在的类，不管是过度设计还是过时了。解决办法：内联类，删除等。
过多的注释：提取方法，重命名类方法。
依恋情节：一个函数对另一个类的兴趣程度远超过自己所在的类。解决办法：挪到另一个类里面去。
过度耦合的消息链：a调b，b调c，c调d只是为了拿个数据。解决办法：隐藏委托，提取方法，移动方法。
中间人：对于委托的滥用。解决办法：内联。
狎昵关系：一个类对另一个类了解太多。解决办法：搬移方法和字段，转换成单向关联，提取公共的类，隐藏委托。
霰弹式修改：一种原因变化多个类，会造成发散式变化。解决办法：移动方法，移动字段，内联。
平行的继承体系：假设有两个类，每给A增加一个子类就要给B增加一个子类。解决办法：移动函数和字段。
当发现某个子类并未带来该有的派生价值时，就需要考虑把他和父类合并（折叠）起来。 
22.2	设计模式拓展思考（3题）7.结构型模式有什么特点？
（描述如何将类或者对象 结合在一起 形成更大的结构）                                                  8. 可以分为哪两类，二者有什么区别？
（类结构型模式vs对象结构型模式；类结构型：关心类的组合，多个类组合成一个更大的系统，一般只存在继承和实现；对象结构型：关心类和对象的组合，尽量使用关联来代替继承，通过关联关系在一个类中定义另一个类的实例对象）

9.适配器模式有几种实现方式，分别是怎么实现的，有什么优缺点？
（对象适配器 vs 类适配器）
（类适配器：
优点：泛化；适配器类是适配者类的子类，可以在适配器类中置换一些适配者的方法，使得适配器灵活性更强；
缺点：一次只能适配一个适配者类，只能为抽象类，不能将一个适配者类与它的子类都适配到目标接口）
（对象适配器：
优点：关联；一个对象适配器，可以把多个不同的适配者 适配到同一个目标，也就是说，同一个适配器可以把适配者类，和它的子类都适配到目标接口；
缺点：想要置换适配者类的方法就不容易）10.外观模式是如何实现的，有什么优缺点？
（将一个系统划分为若干个子系统；子系统间的通信和相互依赖关系达到最小——引入一个外观对象，为子系统的访问提供一个简单而单一的入口；将客户端与子系统的内部复杂性分隔开；）

11.外观模式和适配器模式有什么差别？
1.  行为型模式用于描述多个类或对象之间，怎样相互协作共同完成单个对象都无法单独完成的任务，它涉及算法与对象间职责的分配。能体现出程序在运行时复杂的流程控制。
2.  责任链模式和观察者模式都降低了对象之间的耦合关系，增强了系统的可扩展性。这两种模式完全解耦了吗？
3.  行为型模式分为类行为型模式和对象行为型模式，前者采用继承机制来在类间分派行为，后者采用组合或聚合在对象间分配行为。
（考题）15. 工厂模式，适配器模式，责任链模式和观察者模式属于什么类型的模式？
答案：适配器模式是结构型模式；责任链模式，观察者模式是行为模式；工厂模式是创建型模式22.3	易错题（2题）
关于重构原则，以下说法，哪个是错误的：（C） 
下述针对代码坏味道的重构方法，说法错误的是：(A)
A 某函数存在较多的局部变量，为了减少局部变量的数量，在不改变函数功能的前提下，可以考虑将类型相同的局部变量统一成一个【错误】
B 如果函数或class的唯一用户是test casts（测试用例），这就飘出了坏味道，如果你发现这样的函数或者class，请把他们连同其test case都删掉，但如果这个函数或class是为了测试其他正常功能而存在的，就不能删掉
C 如果switch语句根据类型码进行选择，应该运用提炼函数将switch语句提炼到一个独立函数中，再以搬移函数将它搬移到需要多态性的那个类里
D 当某数组中的元素各自代表的是不同种类的对象时，应该将这些对象用结构体或类等方法进行重新组织，并替换掉数组
新开发过程中需要依赖已有的check函数检查port参数，哪些做法会导致代码劣化:  (ACD)
A. 已有函数check上加一个flag，当flag=true时为新的检查逻辑，flag=false时为旧的检查逻辑
B. 提取新的检查逻辑和旧的检查逻辑相同的部分，新增函数并重构旧函数调用该提取函数
C. 直接拷贝旧函数创建新的检查函数
D. 如果只是新增检查项，可以先调用原有函数，再增加新的检查处理某产品软件有一段【ABD】 某项目对于存量代码进行重构，需要补充测试用例防护网，错误的是（BC）
A、重构过程中对代码有新理解或者发现新的问题时逐步补充新的测试用例【及时补充？？】
B、测试用例可以自动化执行，但是需要开发人员人工确认执行结果是否符合预期
C、要求测试必须补充到非常完善充分以后，才可以修改代码，使得团队在前一大半时间都在补充用例，实际没有重构多少代码
D、在重构代码前先对需要修改的代码补充自动化测试用例下面关于模板方法正确的是：AD
A. 操作的整体步骤固定，其中小部分容易变动的场景
B. 一个请求操作需要多个对象处理时  //责任链模式
C. 为了减少对象创建的数量，降低内存占用和提高性能  //享元模式
D. 各子类中公共的行为需要被提取出来并集中到一个公共的父类中已避免代码重复。 
适合做重构的时机（ABC） 【A注意】题解：代码太混乱，设计错误的 需要重写。针对重构方法，下列说法错误的是（）
...
----------C正确针对重构方法，下列说法错误的是（ABD） 

 
TODO不能留到下个版本实现，这句话是错的
实施重构时应该注重面向不同场景采用不同方法和阵型（ABCD）  
22.4	习题（16题）：
31. 临时字段是指在特定环境下有意义，离开这个环境，它们就什么也不是了，对于临时字段描述错误的是：【提取类、引入特例】
A. 可以通过提炼类将临时字段和操作它们的所有代码提炼到一个类中来重构
B. 将临时字段的所有赋值点都使用函数替换
C. 特定环境相关的代码需要重构到一个集中的地方，避免新增功能出现霰弹式修改
D. 临时字段可能会导致代码可读性变差
 选B
1（单选）一个父类有3个子类，父类中的一个方法只被其中一个子类引用，这时候应该如何进行重构 D
A． 不用重构
B． 把子类合并到父类中
C． 加一个中间类继承父类，将方法移到这个中间类中，然后那个特别的子类再继承这个中间的类
D． 将方法移到子类中 
3（单选）关于重构方法，下列说法错误的有（C）
A．一个函数的某个形参在当前没有被函数内任何代码使用，就应该去掉它。
B. 某个类的getter实例方法，既返回实力状态值，又修改了实例状态，这样违背了命令与查询分离的原则，在使用过程中对于方法的调用者来说，
很容易因为设计与职责不符，造成意外的状态变更产生bug。
C. 如果父类中的某个方法只有少数子类使用，那么可以将这个方法下移到相关子类中去
D. 即使少数子类有代码和功能相同的方法，也必须将这些方法上移到父类中去。4（单选）根据以下哪种白盒测试方法构建防护网，保证重构的质量？C
A.针对待重构模块添加自动化测试【？？？】
B.给系统补充全量自动化测试用例，结果全员在补充自动化用例时花费了很多时间
C.只针对待重构模块进行单元测试防护【？？？】
D.单元测试用例可执行，但是执行结果需要人工确认 
16（多选）如下哪些是内联函数这个重构手法的动机：(CD)
A：提取功能，将部分代码提取到独立函数，方便复用和维护
B：提升可读性，提炼到一个独立的函数中， 并以这段代码的用途为这个函数命名，方便理解
C：消除多余的间接性，将函数和被调用的代码合并了，方便理解
D：重新组织函数的过程中，把多个函数代码内联到一个函数中，方便后续重构22.5	设计模式正交（简单设计）四原则：
1：通过所有测试
2：尽可能消除重复
3：尽可能清晰表达
4：更少的代码元素SOLID原则
SRP  单一职责     一个类，引起它变化的原因只有一个。如果一个变化，导致多处修改，则存在重复。
OCP  开闭原则     对扩展开放，对修改关闭。多个变化导致一处修改，则对变化的识别不准，容易考虑不周，修改引入。
LSP  里氏替换原则  基类设定一系列的规范和契约，子类需要遵守。
ISP  接口分离原则  客户只需要了解必须了解的。不要暴漏的细节。
DIP 依赖倒置原则   接口为稳定的契约，双方都依赖于抽象，不依赖于具体实现；上层定义接口，下层实现接口。老师、学生、班级的UML图
学生和老师是关联关系，学生不能没有老师。
学生和班级是聚合关系，学生个体聚合成班级整体。
学生和心脏是组合关系，心脏是学生总体的局部，组合成学生。强弱关系：泛化(继承) =实现> 组合 > 聚合 > 关联 > 依赖（include）
实线三角 虚线三角 实菱形 空菱形 实线箭头 虚线箭头组合（Composition）： 
是整体与局部的关系，整体的对象负责代表局部的对象的生命周期，可以理解成整体是由局部组成的。例如：借阅记录必须要有相关书籍信息。 
聚合（Aggregation）： 
是整体与个体的关系，可以理解成把个体聚集在一起。例如：借阅者的多条借阅记录。关联（Association）： 
是一种拥有的关系，它使一个类知道另一个类的属性和方法。例如：借阅者的借阅记录或预约记录。依赖（Dependency）： 
是一种使用的关系，有单向依赖和双向依赖，但避免使用双向依赖。例如：借阅者查找结果依赖标题。
22.5.1	习题（10题）：1、（单选）有以下时序图，描述正确的是  C
A．“曹操”“刘备”“诸葛亮”是类，不是对象
B．“攻击”动作会调用“曹操”中的方法
C．图中虚线表示生命线，对象在这一时期内存在
D．图中调用顺序没有先后关系 
2、（单选）一个Manager可以管理多个Employee，一个Employee仅能被一个Manager管理，下面哪个图是对的 A 
6、（多选）以下关系表达方式正确的是（AD）
 C：聚合标识，应该是组合，A　公司由部门组成，不关注比例
D: 依赖 
2.43    对于违反里氏替换原则的两个类A和B，可以采用的候选解决方案中，错误的是 ().LSP  里氏替换原则  基类设定一系列的规范和契约，子类需要遵守。
A. 尽量将一些需要扩展的类或者存在变化的类设计为抽象类或者接口，并将其作为基类，在程序中尽量使用基类对象进行编程【是子类需要更改】
B. 创建一个新的抽象类C，作为两个具体类的超类，将A和B共同的行为移动到C中，从而解决A和B行为不完全一致的问题
C. 将B到A的继承关系改成组合关系
D. 区分是“Is-a”还是“Has-a”。如果是Is-a，可以使用继承关系，如果是Has-a,应该改成组合或聚合关系
选A。10. 面向对象设计的基本原则中的开闭原则指出软件要对扩展开放，对修改关闭。违反开闭原则可能造成的坏味道有（）【OCP原则】
A. 基本类型偏执(Primitive Obsession)
B. 散弹式修改(Shotgun Surgery)
C. 过长参数列表(Long Parameter List)
D. 发散式变化(Divergent Change)
...
23. Visitor（访问者）：数据与行为分离方法。通过这种分离，可达到一个被访问者动态添加新的操作而无需做其他的修改的效果22.6.1	习题（22题）：
11. 设计模式中关于结构型模式，以下说法正确的是
A. 结构型模式可以在不破坏类封装性的基础上，实现新的功能
B. 结构型模式主要用于创建一组对象
C. 结构型模式可以创建一组类的统一访问接口
D. 结构型模式可以在不破坏类封装性的基础上，使得类可以通不曾估计到的系统进行交互
选ACD70、当应用工厂方法模式时要考虑下面哪些问题？
A.必须提供工厂方法的默认实现
B.参数化工厂方法
C.所有语言的特点
D.使用模板以避免创建子类
答案：BCD。工厂方法要避免实现，直观抽象关系。 3（单选）某个系统执行一个动作需要调用数据库查询，比较慢，现在改造成加个缓存模块，然后先调用这个缓存模块，异步从数据库中刷新数据到缓存中，这种做法使用了哪种设计模式 A【？？】
A．代理
B．桥接
C. 适配器
D．享元 
6（多选）采用哪种设计模式，不改变一个第三方库的代码，实现该库功能的对接：BC
A.组合  
B.桥接  
C.适配器  
D.原型
12（单选）要依赖于抽象，不要依赖于具体。即针对接口编程，不要针对实现编程,是( D)的表述
   A.开-闭原则
   B.接口隔离原则
   C.里氏代换原则
   D.依赖倒转原则13（多选）对于依赖倒转的表述正确的是（ABCD）
A. 依赖于抽象而不依赖于具体，也就是针对接口编程。
B. 依赖倒转的接口并非语法意义上的接口，而是，一个类对其他对象进行调用时，所知道的方法集合。
C. 从选项B的角度论述，一个对象可以有多个接口。
D. 实现了同一接口的对象，可以在运行期间，顺利地进行替换。而且不必知道所示用的对象是那个实现类的实例。 
16（单选）下列关于静态工厂与工厂方法表述错误的是：（A）
A.两者都满足开闭原则：静态工厂以if else方式创建对象，增加需求的时候会修改源代码
B.静态工厂对具体产品的创建类别和创建时机的判断是混和在一起的，这点在工厂方法中
C.不能形成静态工厂的继承结构
D.在工厂方法模式中，对于存在继承等级结构的产品树，产品的创建是通过相应等级结构的工厂创建的。开闭原则：对扩展开放，对修改封闭。静态工厂增加需要是修改源代码，对修改不封闭，不符合开闭原则 
17（单选）静态工厂的核心角色是（A）
A.抽象产品
B.具体产品
C.静态工厂
D.消费者18（单选）对象适配器模式是（A）原则的典型应用。
A.合成聚合复用原则 
B.里式代换原则
C.依赖倒转原则 
D.迪米特法则19（单选）应用设计模式的一个优点是：B
A：程序易于理解
B：适应需求变化
C：减少代码量
D：简化软件系统设计 
...
答案：B错误（这个题看一下四原则的解释）
解析：正交四原则是指简单设计的四个基本原则，包括1）通过所有测试 2）尽可能消除重复 3）尽可能清晰表达 4）更少的代码元素
通过所有测试是说的通过了客户验收的标准而不是通过所有测试。 
7.下面代码适合用什么手法进行重构（）答案：“函数搬移Move”:函数应该放在它所使用的数据的所属对象内，所以amountFor()应该移到Rental类去，要运用Move Method手法。23	可信设计（13题）
23.1	身份和访问控制管理23.1.1	身份管理
账号管理 
口令管理 
不可用AES-256-ECB，用AES-256-CBC
口令传输
口令在非信任网络之间进行传输须采用安全传输通道或者加密后传输，有标准协议规定除外。 23.1.2	认证
认证方法选C认证接口安全管理
1.在登录认证过程中，下面哪项是最有效的口令防暴力破解消减措施
A.定期更新账号口令
B.增加认证验证码校验
...
关于隐私，下列说法错误的是：(C)
A 个人数据不一定是隐私
B 隐私包括个人不愿他人知晓的信息和不愿他人侵犯的空间
C 个人数据是隐私的子集，隐私包含个人数据
D 隐私和个人数据是两个独立的概念23.4.1	隐私保护背景
隐私保护越来越引起大家重视，对于GDPR描述错误的是：D 
选D23.4.2	隐私保护设计相关概念
系统审计日志中打印了用户执行操作命令的时间、用户ID、事件类型，命令结果等信息，以支持日志回溯审计【这个打印是可以的】第三方：
•       指数据主体、数据控制者、数据处理者以及根据数据控制者或者处理者的直接授权而处理数据的人之外的任何自然人或法人、公共权力机关、代理机构或其他机构。管理员没有删除日志的权限
23.4.3	隐私保护设计要求【内容比较多】以下关于隐私数据相关设计的描述，哪些设计是错误的(BC)
A. 匿名化是一种对数据不可逆改变的过程，处理后的个人数据将无法直接或间接地还原，已经不再是个人数据，因此匿名化后的数据不受个人数据保护法律法规的限制。
B. 假名化是一种数据保护的手段（如偏移、置换、哈希等），假名化数据不是个人数据，因此假名化以后的数据不受个人数据保护法律法规的限制。
C. 考虑易用性，系统默认打开个人数据收集便于启动个性化运维功能，用户可以通过设置关闭。
D. 对于需要存储的个人数据，在设计时应根据收集和使用目的确定存留期。
 23.5	密码学算法应用
23.5.1	密码算法随机数正确答案：盐值、会话ID、CBC加密的IV
序列号也是不需要的