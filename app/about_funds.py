...
async def get_one_fund(code) -> dict:
    # 基金当前信息
...
async def get_funds_info(num: int):
    ...
    df_info = pd.DataFrame(infos).rename(columns=name_mapping)
...
if __name__ == '__main__':
    ...
    df.to_excel(r'D:\python\{}.xlsx'.format(datetime.date.today().strftime('%Y%m%d')), index=False)
    print('done')